...
export default function IntegratedNetworkView({
    ...
    requestAnimationFrame(() => {
      try {
        ...
        const labelGroups = labelsLayer
          ...
          .style('opacity', 0)
          .style('transition', `opacity ${NETWORK_STYLES.text.fadeTransition}ms ease-in-out`)
        ...
        .style('position', 'sticky')  // Changed from 'absolute' to 'fixed'
        .style('inset', '0 0 auto auto')     // Position from bottom
        .style('background', NETWORK_STYLES.tooltip.background)
        .style('opacity', 0)
        .style('z-index', NETWORK_STYLES.tooltip.zIndex)
        .style('pointer-events', 'none')
        .style('transition', 'opacity 300ms ease-in-out');  // Smooth transition
        ...
        nodeCircles.on('mouseover', function(event, d) {
          ...
          // Apply enhanced label visibility logic based on zoom level
          if (currentScale < NETWORK_STYLES.text.veryZoomedOutThreshold) {
            // Very zoomed out - only show the hovered node's label with animation
            labelGroups.style('opacity', nodeData => nodeData.id === d.id ? 1 : 0);
          } else {
            // At higher zoom levels - show labels for hovered & connected nodes with animation
            labelGroups.style('opacity', nodeData => 
              connectedNodeIds.has(nodeData.id) || nodeData.id === d.id ? 1 : 0
            );
          }
          
          // Ensure text is sized consistently regardless of zoom level
          textLabels.style('font-size', `${NETWORK_STYLES.text.fontSize / currentScale}px`);
          
          // Update background rectangles to match text
        ...
        nodeCircles.on('mouseout', function(event, d) {
          ...
          // Get current zoom scale directly
          const currentTransform = d3.zoomTransform(rootSvg.node());
          const currentScale = currentTransform.k;
          
          // Restore label visibility based on zoom thresholds
          if (currentScale < NETWORK_STYLES.text.veryZoomedOutThreshold) {
            // When very zoomed out, hide all labels on mouseout
            labelGroups.style('opacity', 0);
          } else {
            // At higher zoom levels, show labels that meet visibility threshold
            labelGroups.style('opacity', nodeData => 
              shouldLabelBeVisible(nodeData.type, currentScale) ? 1 : 0
            );
          }
        ...
        const updateLabelVisibility = (scale) => {
          ...
          // Determine visibility based on zoom thresholds
          if (scale < NETWORK_STYLES.text.veryZoomedOutThreshold) {
            // Very zoomed out - hide all labels (they'll only show on hover)
            labelGroups.style('opacity', 0);
          } else {
            // Normal zoom behavior - show labels that meet threshold
            labelGroups.style('opacity', nodeData => 
              shouldLabelBeVisible(nodeData.type, scale) ? 1 : 0
            );
          }
  ...
  }, [filteredData, dimensions, isClient, clusterSeparation, layoutStabilized]);
    ...
    <div className="network-graph" ref={containerRef}>
      ...
      {/* <button 
        ...
        onClick={() => {
          ...
          // Force a re-render by updating data
          setFilteredData(prevData => ({
            // Create a fresh copy to trigger re-render
      ...
      {/* <div className="network-stats">
        <div className="network-stats__item">
          <strong>Bottlenecks:</strong> {filteredData.nodes.filter(n => n.type === 'bottleneck').length}
        </div>
        <div className="network-stats__item">
          <strong>Capabilities:</strong> {filteredData.nodes.filter(n => n.type === 'capability').length}
        </div>
        {showResources && (
          <div className="network-stats__item">
            <strong>Resources:</strong> {filteredData.nodes.filter(n => n.type === 'resource').length}
            {resourceLimit > 0 && resources.length > resourceLimit && (
              <span className="network-stats__limited"> (of {resources.length} total)</span>
            )}
          </div>
        )}
        <div className="network-stats__item">
          <strong>Links:</strong> {filteredData.links.length}
        </div>
      </div> */}
      
      {/* <div className="network-legend">
        <div className="network-legend__title">Legend</div>
        <div className="network-legend__item">
          <div className="network-legend__color" style={{ backgroundColor: 'var(--field-color-0)', width: NETWORK_STYLES.nodeRadius.bottleneck + 'px', height: NETWORK_STYLES.nodeRadius.bottleneck + 'px', borderRadius: '50%' }}></div>
          <div className="network-legend__label">Bottlenecks (colored by field)</div>
        </div>
        <div className="network-legend__item">
          <div className="network-legend__color" style={{ backgroundColor: NETWORK_STYLES.nodeColors.capability, width: NETWORK_STYLES.nodeRadius.capability + 'px', height: NETWORK_STYLES.nodeRadius.capability + 'px', borderRadius: '50%' }}></div>
          <div className="network-legend__label">Capabilities</div>
        </div>
        {showResources && (
          <div className="network-legend__item">
            <div className="network-legend__color" style={{ backgroundColor: NETWORK_STYLES.nodeColors.resource, width: NETWORK_STYLES.nodeRadius.resource + 'px', height: NETWORK_STYLES.nodeRadius.resource + 'px', borderRadius: '50%' }}></div>
            <div className="network-legend__label">Resources</div>
          </div>
        )}
      </div> */}
      ...
      <div className="network-instructions">
        ...
        <div className="zoom-indicator">
          ...
          {/* <div className="zoom-levels">
            <div className={`zoom-level ${currentZoomScale >= NETWORK_STYLES.text.zoomThresholds.bottleneck ? 'active' : ''}`}>
              Bottleneck labels
            </div>
            <div className={`zoom-level ${currentZoomScale >= NETWORK_STYLES.text.zoomThresholds.capability ? 'active' : ''}`}>
              Capability labels
            </div>
            <div className={`zoom-level ${currentZoomScale >= NETWORK_STYLES.text.zoomThresholds.resource ? 'active' : ''}`}>
              Resource labels
            </div>
          </div> */}
        </div>
